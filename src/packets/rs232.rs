...
trait Packet {
    ...
    fn set_data(&mut self, data: &[u8]) -> &mut Self {
        ...
        self.get_mut_buf()[SIZE_INDEX] =
            (data.len() + Self::DATA_BEGIN_INDEX - LEN_OF_HEADER + 1) as u8;
        self
    }
    ...
    fn write(&mut self, writer: &mut dyn WriteExt) -> io::Result<()> {
        let len = self.len();
        self.get_mut_buf()[len - 1] = write_packet(
            writer,
            self.dest(),
            &self.get_mut_buf()[LEN_OF_HEADER..len - 1],
        )?;
        Ok(())
    }
}
...
pub fn read_packet(reader: &mut dyn ReadExt, buf: &mut [u8]) -> io::Result<usize> {
    ...
    while counter < buf[SIZE_INDEX] as usize {
        buf[LEN_OF_HEADER..][counter] = reader.read_u8_escaped()?;
        counter += 1;
    }
...
/// Returns checksum
pub fn write_packet(writer: &mut dyn WriteExt, dest: u8, data: &[u8]) -> io::Result<u8> {
    let mut sum: u8 = 0;

    writer.write_u8(SYNC)?;
    writer.write_u8_escaped(dest)?;
    writer.write_u8_escaped((data.len() as u8).wrapping_add(1))?;
