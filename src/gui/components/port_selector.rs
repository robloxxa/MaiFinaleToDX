...
/// A combo box populated with available COM ports.
/// Shows a refresh button ("↺") next to it to re-scan ports on demand.
...
pub fn port_combobox(ui: &mut egui::Ui, id_salt: impl std::hash::Hash, port: &mut String) -> bool {
    ...
    ui.horizontal(|ui| {
        egui::ComboBox::from_id_salt(egui::Id::new(id_salt))
            .selected_text(port.as_str())
            .width(70.0)
            .show_ui(ui, |ui| {
                for p in &ports {
                    changed |= ui.selectable_value(port, p.clone(), p).changed();
                }
                // Keep current value selectable even if not detected
                if !ports.contains(port) && !port.is_empty() {
                    let current = port.clone();
                    ui.selectable_value(port, current.clone(), current.as_str());
                }
            });
